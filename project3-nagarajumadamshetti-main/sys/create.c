...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	proctab[pid].pdbr = (frame_id + FRAME0) * NBPG; // Set the Page Directory Base Register for the process
	frm_tab[frame_id] = (fr_map_t){ .fr_status = FRM_MAPPED, .fr_type = FR_DIR, .fr_pid = pid }; // Mark frame as mapped and of type 'FR_DIR'

	pd_t *page_directory_entry; // Declare a pointer to page directory entry
	page_directory_entry = proctab[pid].pdbr; // Initialize the page directory entry pointer
	i=0;
	while (i < 4) {
		// Set up the first four page directory entries as present
		page_directory_entry[i] = (pd_t){.pd_pres = 1, .pd_base = FRAME0 + i};
		i += 1;
	}

	i = 0;
	while (i < 1024) {
		// Set write permission for all 1024 page directory entries
